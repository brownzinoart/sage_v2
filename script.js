...
class SageFormHandler {
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      // Connection recovery: Try to establish and validate connection
      console.log('Testing Ollama connection...');
      const connectionTest = await this.testConnectionWithRecovery();
      ...
      const responseData = {
        ...
        benefits,
        products,
        host: (window.Config && window.Config.get('OLLAMA_HOST')) || null,
        timestamp: Date.now(),
        isDemo: false
      };
  ...
  // Enhanced connection testing with auto-recovery
  async testConnectionWithRecovery() {
    ...
    if (connectionResult.success) {
      console.log('✅ Current connection working');
      return { success: true, host: connectionResult.host };
    }
    
    console.warn(`❌ Current connection failed: ${connectionResult.error}`);
    
    // Show user-friendly notification about attempting auto-discovery
    const statusDiv = this.showConnectionStatus('Attempting to find working Ollama connection...', 'warning');
    
    // Try auto-discovery
    const autoDiscovery = await window.Config.autoDiscoverHost();
    
    if (autoDiscovery.success) {
      this.updateConnectionStatus(statusDiv, `✅ Connected to Ollama at ${autoDiscovery.host}`, 'success');
      setTimeout(() => statusDiv.remove(), 3000);
      return { success: true, host: autoDiscovery.host };
    }
    
    // If all else fails, show error with helpful guidance
    this.updateConnectionStatus(statusDiv, '❌ Cannot connect to Ollama server', 'error');
    this.showConnectionGuidance(statusDiv);
    
    return { success: false, error: 'No working Ollama connection found' };
  }
  ...
  updateConnectionStatus(statusDiv, message, type) {
    statusDiv.innerHTML = message;
    statusDiv.className = `connection-status ${type}`;
  ...
  // Show connection guidance to user
  showConnectionGuidance(statusDiv) {
    ...
    setTimeout(() => {
      statusDiv.innerHTML = `
        <div style="text-align: left; line-height: 1.5;">
          ...
          <small>Current target: ${currentHost}</small><br><br>
          <strong>Quick fixes:</strong><br>
          1. Make sure Ollama is running<br>
          2. Open DevTools Console and try:<br>
          <code style="background: rgba(0,0,0,0.2); padding: 2px 4px; border-radius: 3px;">window.Config.autoDiscoverHost()</code><br>
          3. Or manually set host:<br>
          <code style="background: rgba(0,0,0,0.2); padding: 2px 4px; border-radius: 3px;">window.Config.set('OLLAMA_HOST', 'http://localhost:11434')</code>
        </div>
      `;
