...
class SageAPI {
  ...
  async generateResponse(userInput, experienceLevel, useModel = null) {
    ...
    try {
      ...
      // Enhanced connectivity probe with auto-discovery fallback
      let connectionResult = await window.Config.testConnection();
      
      if (!connectionResult.success) {
        console.warn(`Initial connection failed: ${connectionResult.error}`);
        console.log('Attempting auto-discovery...');
        
        const autoDiscovery = await window.Config.autoDiscoverHost();
        if (autoDiscovery.success) {
          console.log(`✅ Auto-discovery found working host: ${autoDiscovery.host}`);
          // Update ollamaHost to use the discovered host
          const updatedHost = window.Config.get('OLLAMA_HOST');
          if (updatedHost !== ollamaHost) {
            console.log(`Updating host from ${ollamaHost} to ${updatedHost}`);
          }
        } else {
          console.error('❌ Auto-discovery failed, using fallback response');
          throw new Error(`Cannot reach Ollama server. Tried multiple hosts. Last error: ${connectionResult.error}`);
        }
      } else {
      ...
      // Enhanced fallback with connection guidance
      console.warn('Falling back to generic response');
      console.log('Error details for user guidance:', {
        isConnectionError: error.message.includes('Cannot reach') || error.message.includes('CORS'),
        isCorsError: error.message.includes('CORS') || error.message.includes('cross-origin'),
        isTimeoutError: error.message.includes('timeout') || error.message.includes('AbortError'),
        currentHost: window.Config.get('OLLAMA_HOST')
      });
      
      return this.getFallbackResponse(userInput, experienceLevel, error);
    }
  }
...
Each section should be 2-4 sentences and provide authoritative, clinical-grade guidance. Be specific about strains, dosages, and mechanisms. Write with confidence - no hedging or uncertainty.`;
  ...
  getFallbackResponse(userInput, experienceLevel, error = null) {
    // Enhanced fallback response with connection guidance
    console.warn('Using fallback response - Ollama server connection failed');
    
    const currentHost = window.Config.get('OLLAMA_HOST');
    let connectionGuidance = '';
    
    if (error) {
      if (error.message.includes('CORS') || error.message.includes('cross-origin')) {
        connectionGuidance = `\n\n🔧 Connection Issue: CORS error with ${currentHost}. Try opening DevTools Console and run: window.Config.autoDiscoverHost()`;
      } else if (error.message.includes('Cannot reach')) {
        connectionGuidance = `\n\n🔧 Connection Issue: Cannot reach Ollama at ${currentHost}. Make sure Ollama is running or try: window.Config.set('OLLAMA_HOST', 'http://localhost:11434')`;
      } else if (error.message.includes('timeout')) {
        connectionGuidance = `\n\n🔧 Connection Issue: Request timeout to ${currentHost}. The server may be slow or unresponsive.`;
      }
    }
    
    const fallbacks = {
      new: `[INTRODUCTION]\nI understand you're looking for personalized cannabis guidance as someone new to cannabis.\n\n[CORE_TOPIC]\nFor beginners, I recommend starting with CBD-dominant products (2.5-5mg THC maximum) from licensed dispensaries. Focus on clearly labeled, tested products with predictable effects.\n\n[WHY_THIS_WORKS]\nLow THC doses allow you to gauge your tolerance safely while CBD provides therapeutic benefits without overwhelming psychoactive effects.\n\n[WHAT_TO_EXPECT]\nExpect mild, gentle effects within 30-90 minutes. You may feel relaxed, slightly euphoric, or notice reduced anxiety without intense impairment.\n\n[GETTING_STARTED]\nVisit a licensed dispensary, explain you're new, start with the lowest dose, and wait at least 2 hours before taking more.${connectionGuidance}`,
      
      casual: `[INTRODUCTION]\nGreat question! Based on your casual experience with cannabis, I can provide some targeted guidance.\n\n[CORE_TOPIC]\nFor casual users, indica-dominant hybrids work well for relaxation while sativas provide energy and focus. Look for products in the 5-15mg THC range depending on your tolerance.\n\n[WHY_THIS_WORKS]\nYour existing tolerance allows for moderate doses, and hybrids provide balanced effects that work well for various activities and times of day.\n\n[WHAT_TO_EXPECT]\nEffects typically onset within 15-60 minutes depending on consumption method, lasting 2-4 hours with peak effects in the first hour.\n\n[GETTING_STARTED]\nChoose products based on desired effects, start with familiar doses, and consider your planned activities when selecting strains.${connectionGuidance}`,
      
      experienced: `[INTRODUCTION]\nAs an experienced user, you're looking for sophisticated cannabis guidance that matches your knowledge level.\n\n[CORE_TOPIC]\nFocus on cannabinoid profiles (CBG, CBN, THCV), terpene interactions, and craft cultivars. Consider minor cannabinoids for targeted effects and premium extraction methods.\n\n[WHY_THIS_WORKS]\nYour experience allows you to appreciate complex profiles and subtle differences in cultivars, terpenes, and extraction techniques that newer users can't distinguish.\n\n[WHAT_TO_EXPECT]\nWith your tolerance, expect nuanced effects from terpene profiles and cannabinoid ratios. Peak effects depend on consumption method and your current tolerance.\n\n[GETTING_STARTED]\nSeek out craft producers, experiment with different terpene profiles, consider tolerance breaks, and explore minor cannabinoid products for specific effects.${connectionGuidance}`
    };
  ...
  async buildProductSearchSpec(userInput, experienceLevel) {
    ...
    try {
      ...
      // Quick connectivity probe to avoid hanging this optional step
      try {
        const ollamaHost = window.Config.get('OLLAMA_HOST');
        if (typeof AbortController !== 'undefined') {
          const controller = new AbortController();
        ...
        // Skip model-assisted spec if probe fails
        return baseSpec;
      }
